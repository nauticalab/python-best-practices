...
- **Stdout only.** `print()` always writes to **stdout** (standard output — the default stream for normal program output). Many production environments expect errors and warnings on **stderr** (standard error — a separate stream conventionally reserved for diagnostics, so that errors can be captured or redirected independently of normal output). Splitting output across both streams with `print()` requires manual `file=sys.stderr` arguments everywhere; the `logging` module routes the same log call to as many destinations as you like — stdout, stderr, a rotating file, *and* a cloud log service — just by configuring handlers.
